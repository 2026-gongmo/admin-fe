...
CSV 다운로드 API는 1차로 연결했고, PDF 생성, 파일 업로드, 공공데이터 전체 배치 수집, AI 실제 연동은 2차 이후로 두는 편이 현실적입니다.
...
| PDF 생성 | POST | `/api/admin/monthly-reports/{yearMonth}/exports/pdf` | 아직 구현 안 됨 · 추가 예정 |
...
| 단계 | 포함 기능 | 제외 기능 |
|---|---|---|
| 1차 | 관리자 인증/인가, Campus/Building seed, 접근성 제보 조회/상태 변경/담당자/우선순위/메모, 감사 로그 | AI 실제 연동, PDF 생성, 공공데이터 배치 |
| 2차 | 도움 요청 조회/상태 변경, 경험 피드 검수/공개 상태, 개선 워크플로우 단계 변경 | 실시간 push, 파일 업로드 고도화 |
| 3차 | 대시보드 집계, 공공데이터 비교 저장, 월간 리포트 스냅샷 | 실내 내비게이션, 이미지 분석 모델 |
...
- AI 익명화 실제 API 연결은 별도 보안 리뷰 후 진행합니다.
